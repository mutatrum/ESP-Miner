...
void POWER_MANAGEMENT_task(void * pvParameters)
{
    ...
    BoardConfig * BOARD_CONFIG = &GLOBAL_STATE->BOARD_CONFIG;
    ...
    while (1) {
        ...
        POWER_MANAGEMENT_MODULE->voltage = Power_get_input_voltage(BOARD_CONFIG);
        POWER_MANAGEMENT_MODULE->power = Power_get_power(BOARD_CONFIG);

        POWER_MANAGEMENT_MODULE->fan_rpm = Thermal_get_fan_speed(BOARD_CONFIG);
        POWER_MANAGEMENT_MODULE->chip_temp_avg = GLOBAL_STATE->ASIC_initalized ? Thermal_get_chip_temp(BOARD_CONFIG) : -1;

        POWER_MANAGEMENT_MODULE->vr_temp = Power_get_vreg_temp(BOARD_CONFIG);
        ...
        if ((POWER_MANAGEMENT_MODULE->vr_temp > TPS546_THROTTLE_TEMP || POWER_MANAGEMENT_MODULE->chip_temp_avg > THROTTLE_TEMP) && (POWER_MANAGEMENT_MODULE->frequency_value > 50 || POWER_MANAGEMENT_MODULE->voltage > 1000)) {
            ...
            Thermal_set_fan_percent(BOARD_CONFIG, 1);

            // Turn off core voltage
            VCORE_set_voltage(BOARD_CONFIG, 0.0f);
        ...
        if (nvs_config_get_u16(NVS_CONFIG_AUTO_FAN_SPEED, 1) == 1) {
            if (POWER_MANAGEMENT_MODULE->chip_temp_avg >= 0) { // Ignore invalid temperature readings (-1)
                ...
                Thermal_set_fan_percent(BOARD_CONFIG, pid_output / 100.0);
                ...
                if (GLOBAL_STATE->SYSTEM_MODULE.ap_enabled) {
                    ...
                    Thermal_set_fan_percent(BOARD_CONFIG, 0.7);
                } else {
            ...
            Thermal_set_fan_percent(BOARD_CONFIG, (float) fs / 100.0);
        }
        ...
        if (core_voltage != last_core_voltage) {
            ...
            VCORE_set_voltage(BOARD_CONFIG, (double) core_voltage / 1000.0);
