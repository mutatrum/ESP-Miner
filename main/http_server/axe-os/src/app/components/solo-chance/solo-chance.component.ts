...
export class SoloChanceComponent implements OnInit, OnDestroy {
  ...
  private generateRows(info: ISystemInfo): void {
    ...
    if (info.totalUptimeSeconds ?? 0 > info.uptimeSeconds) {
      const expectedReachedTotalDifficulty = this.calculateExpectedReachedDifficulty(hashRate, info.totalUptimeSeconds ?? 0);
      if (expectedReachedTotalDifficulty) {
        difficulties.push({
          value: expectedReachedTotalDifficulty,
          label: '📅 Total Uptime',
          tooltip: 'Expected difficulty reached with current hashrate and uptime',
        });
      }
