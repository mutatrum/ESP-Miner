...
esp_err_t VCORE_init(BoardConfig * BOARD_CONFIG) {
    ...
    if (BOARD_CONFIG->TPS546) {
        switch (BOARD_CONFIG->device.asic_count) {
            case 1:
    ...
    if (BOARD_CONFIG->plug_sense) {
        ...
        if (barrel_jack_plugged_in == 1 || BOARD_CONFIG->asic_enable) {
...
esp_err_t VCORE_set_voltage(BoardConfig * BOARD_CONFIG, float core_voltage)
{
    ...
    if (core_voltage == 0.0f && BOARD_CONFIG->asic_enable) {
        gpio_set_level(GPIO_ASIC_ENABLE, 1);
    }
