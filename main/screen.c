...
static lv_obj_t * create_scr_bitaxe_logo() {
    lv_obj_t * scr = lv_obj_create(NULL);
    ...
    lv_obj_t *label1 = lv_label_create(scr);
    lv_label_set_text(label1, DEVICE_CONFIG->family.name);
    lv_obj_align(label1, LV_ALIGN_RIGHT_MID, -6, -12);

    lv_obj_t *label2 = lv_label_create(scr);
    lv_label_set_text(label2, DEVICE_CONFIG->board_version);
...
esp_err_t screen_start()
{
    ...
    if (SYSTEM_MODULE->is_screen_active) {
        ...
        screens[SCR_BITAXE_LOGO] = create_scr_bitaxe_logo();
